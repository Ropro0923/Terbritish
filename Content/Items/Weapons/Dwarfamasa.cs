...
namespace Terbritish.Content.Items.Weapons
{
    public class Dwarfamasa : ModItem
    {
        ...
        {
            if (type == ModContent.ProjectileType<DwarfamasaStab>())
            {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            {
                Projectile.NewProjectile(source, position, velocity*3.25f, ModContent.ProjectileType<DwarfamasaThrown>(), (int)(damage * 0.67f), knockback, player.whoAmI);
                return false;
            }
            return true;
        }
